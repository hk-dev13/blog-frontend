      ...
      <aside className={`hidden md:flex flex-col p-4 transition-all duration-300 ${isSidebarCollapsed ? 'w-20' : 'w-72'}`}>
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-slate-200/70 bg-white/80 shadow-sm backdrop-blur dark:border-slate-800/70 dark:bg-slate-900/60">
          <div className="relative overflow-hidden border-b border-slate-200/70 px-4 py-4 dark:border-slate-800/70 flex items-center justify-between">
            ...
            <div className="relative overflow-hidden">
              <Link href="/admin" className="text-xl font-bold font-serif text-slate-950 dark:text-white block truncate">
                {isSidebarCollapsed ? 'E' : 'Editor'}
              </Link>
              {!isSidebarCollapsed && <p className="text-xs text-slate-500 dark:text-slate-400">Admin Panel</p>}
            </div>
            <button
              type="button"
            ...
            {filteredNavItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`group flex items-center gap-2 px-3 py-2 rounded-xl transition-all font-semibold ${isSidebarCollapsed ? 'justify-center' : ''} ${isActive
                ...
                >
                  <span className={`grid h-7 w-7 place-items-center rounded-xl border transition-colors ${isActive
                  ...
                  >
                    <item.icon className={`w-3.5 h-3.5 ${isActive ? 'text-slate-900 dark:text-slate-100' : 'text-slate-500 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white'}`} />
                  </span>
          ...
          <div className="p-3 border-t border-slate-200/70 dark:border-slate-800/70">
            <div className={`flex items-center gap-2 rounded-2xl border border-slate-200/70 bg-white/70 px-3 py-2 mb-2 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/40 ${isSidebarCollapsed ? 'justify-center' : ''}`}>
            ...
            <button
              ...
              className={`flex items-center gap-2 w-full px-3 py-1.5 text-slate-700 dark:text-slate-500 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400 rounded-xl transition-colors font-semibold text-left text-xs ${isSidebarCollapsed ? 'justify-center' : ''}`}
            >
